...
class DailyExpenditureSummarySheetController extends Controller
{
    ...
    public function printDailyExpenditure(Request $request){
        ...
        $data['expen'] = DB::select("select t.id, t.l_name, t.bank_name, t.v_no, sum(t.amount) as amount
         from(
        SELECT l.id, l.name as l_name,bk.name as bank_name,v.voucher_number as v_no, vd.amount FROM `lnames` AS l
        JOIN `ltypes` AS lt ON lt.id = l.ltype_id
        JOIN `voucher_details` AS vd ON vd.lname_id = l.id
        JOIN `vouchers` AS v ON v.id = vd.voucher_id
        JOIN `banks` AS bk ON bk.id = v.bank_id
        WHERE (v.voucher_date BETWEEN '".$from_date."' AND '".$to_date."') AND (l.ltype_id = 3 OR l.ltype_id = 2) AND v.project_id = ".$project_id.") as t
        group by t.id");
